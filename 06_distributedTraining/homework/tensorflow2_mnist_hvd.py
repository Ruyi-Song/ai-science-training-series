...
t0 = time.time()
nstep = nsamples//(args.batch_size * parallel_threads)
ntest_step = ntests//(args.batch_size * parallel_threads)
metrics={}
metrics['train_acc'] = []
metrics['valid_acc'] = []
metrics['train_loss'] = []
...
for ep in range(args.epochs):
    ...
    #Ruyi hvd start
    training_loss_syn = hvd.allreduce(training_loss, average=True)
    training_acc_syn = hvd.allreduce(training_acc, average=True)
    #Ruyi hvd end
    ...
    #Ruyi hvd start
    test_loss_syn = hvd.allreduce(test_loss, average=True)
    test_acc_syn = hvd.allreduce(test_acc, average=True)
    #Ruyi hvd end

    tt1 = time.time()
    print('E[%d], train Loss: %.6f, training Acc: %.3f, val loss: %.3f, val Acc: %.3f\t Time: %.3f seconds' % (ep, training_loss_syn, training_acc_syn, test_loss_syn, test_acc_syn, tt1 - tt0))
    ...
    metrics['train_acc'].append(training_acc_syn.numpy())
    metrics['train_loss'].append(training_loss_syn.numpy())
